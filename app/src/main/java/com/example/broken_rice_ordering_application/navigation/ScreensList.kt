package com.example.broken_rice_ordering_application.navigation

enum class ScreensList(val route:String) {
    TABNAV("tabNav"),
    FOODTYPE_MN_SCREEN("food_type_mn_screen"),
    FOOD_MN_SCREEN("food_mn_screen"),
    ADD_FOODTYPE_SCREEN("add_food_type_screen"),
    UPDATE_FOODTYPE_SCREEN("update_food_type_screen"),
    FOODTYPE_LIST_SCREEN_Update("food_type_list_screen_update"),
    FOODTYPE_LIST_SCREEN_Delete("food_type_list_screen_delete"),
    ADD_FOOD_SCREEN("add_food_screen"),
    UPDATE_FOOD("update_food_screen"),
}